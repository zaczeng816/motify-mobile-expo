...
function ModifyChallengeModal({ isModalVisible, hideModal, isNew, challenge }) {
    ...
    const [category, setCategory] = isNew
        ? useState(categories[0])
        : useState(challenge.category);
    const [isPrivate, setIsPrivate] = isNew
        ? useState(true)
        : useState(challenge.isPrivate);
    const [type, setType] = isNew
        ? useState("habit")
        : useState(challenge.type);
    const [frequency, setFrequency] = isNew
        ? useState("day")
        : useState(challenge.frequency);
    const [isTimeBased, setIsTimeBased] = isNew
        ? useState(false)
        : useState(challenge.isTimeBased);
    const [duration, setDuration] = isNew
        ? useState(new Date(0, 0, 0, 0, 0, 0))
        : useState(challenge.duration);
    const [amount, setAmount] = isNew
        ? useState("")
        : useState(challenge.amount);
    const [unit, setUnit] = isNew ? useState("") : useState(challenge.unit);
    const [description, setDescription] = isNew
        ? useState("")
        : useState(challenge.description);
    const [isOngoing, setIsOngoing] = isNew
        ? useState(false)
        : useState(challenge.isOngoing);
    const [startDate, setStartDate] = isNew
        ? useState(new Date())
        : useState(challenge.startDate);
    const [endDate, setEndDate] = isNew
        ? useState(new Date())
        : useState(challenge.endDate);
    ...
    const handleSubmit = async () => {
        ...
        console.log("handleSubmit: ", newChallenge);
        try {
            setLoading("Creating challenge...");
            const response = await createChallenge(token, newChallenge);
            if (response) {
            ...
            console.log("handleSubmit: ", e.message);
            showMessage("Cannot Modify Challenge", e.message);
        }
        return;
    ...
    return (
        ...
        >
            ...
            <KeyboardAwareScrollView style={styles.container}>
                ...
                />
                <PickerModal
                    scrollModalVisible={scrollModalVisible}
